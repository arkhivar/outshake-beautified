...
- Offers a fixed light theme, profile selection, an on/off switch and a Quick
...
- **Background shake:** no wake lock; sensor delivery, foreground-service starts,
  sticky restart and boot behavior depend on Android/OEM restrictions. There is
  no guarantee while asleep, force-stopped, power-restricted or after consent is
  revoked. Sensitivity changes currently require stopping/restarting shake mode.
...
- **Shake does nothing:** first connect from the app and grant VPN consent. Check
  ...
  cooldown. Toggle shake off/on after changing sensitivity. Confirm the device
